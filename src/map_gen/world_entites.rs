...
impl TranslateBrush {
    ...
    pub fn update(
        ...
        mut commands: Commands,
        mut query: Query<
            (Entity, &mut TranslateBrush, &mut Transform, &RigidBody),
            With<BrushEntity>,
        >,
    ) {
        for (ent, mut tb, mut t, _) in &mut query {
            if tb.time == 0.0 {
                ...
                // let diff = t.translation.lerp(tb.goal, tb.cur_time / tb.time) - t.translation;
